...
void ImGuiConsole::DrawIfVisible(const char* title, ConsoleMode consoleMode)
{
	...
	if (ImGui::BeginMenuBar())
	{
		if (showMenuBar)
		{
			...
			{
				ImGui::SliderFloat("半透明时的不透明度", &window_bgAlphaWhenSemi, 0.1f, 1.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp);
				...
				ImGui::SliderFloat("高度", &windowBaseHeight, 100.0f, windowBaseHeightMax, "%.3f", ImGuiSliderFlags_AlwaysClamp);
				ImGui::EndMenu();
			}
			ImGui::Checkbox("自动滚动", &AutoScroll);
			ImGui::Separator();
			ImGui::Checkbox("刷屏", &doSpamInConsole);
			ImGui::Separator();
		}
	...
	{
		if (ImGui::Selectable("复制此行"))
		{
			ImGui::SetClipboardText(m_SingleLineToCopy.c_str());
		}
		if (ImGui::Selectable("复制全部")) copy_to_clipboard = true;
	...
	if (showFooter)
	{
		...
		if (ImGui::SmallButton("添加调试文本")) { AddLogF("%d some text", Items.Size); AddLogF("some more text"); AddLogF("display very important message here!"); }
		ImGui::SameLine();
		if (ImGui::SmallButton("添加调试错误")) { AddLogF("[error] something went wrong"); }
		ImGui::SameLine();
