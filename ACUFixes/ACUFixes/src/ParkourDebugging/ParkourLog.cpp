...
void ParkourLog::DrawDisplayControls()
{
    ...
    ImGui::Checkbox("立即弃用的（有时可能影响性能）", &m_DisplaySettings.m_ShowDiscardedEarly);
    ImGui::Checkbox("后期弃用的", &m_DisplaySettings.m_ShowDiscardedLate);
    ImGui::Checkbox("未弃用的", &m_DisplaySettings.m_ShowNondiscarded);
    ImGui::NewLine();
    ImGui::Checkbox("显示多个最近的循环（有时可能影响性能）", &m_DisplaySettings.m_ShowMoreThanOneOfTheRecentCycles);
    {
        ImGuiCTX::Indent _ind;
        ImGui::Checkbox("跳过完全弃用的循环", &m_DisplaySettings.m_ShowMoreThanOneOfTheRecentCycles_SkipAllDiscarded);
    }
