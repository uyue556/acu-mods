...
void ParkourDebugWindow::DrawModWeightSlider(std::optional<float>& modWeight)
{
    ...
    if (!modWeight)
    {
        if (ImGui::Button("启用", ImVec2(-FLT_MIN, 0.0f)))
        {
...
#define MDCOL(colId, name) MoveDetailsColumnsIndices::colId, name, DrawCol_##colId
    return std::make_tuple(
            MoveDetailsColumn{ MDCOL(Index, "序号"), SortByAttribute{[](Action_t& a) { return a->m_Index; }} }
            , MoveDetailsColumn{ MDCOL(Type, "类型ID"), SortByAttribute{ [](Action_t& a) { return a->m_ActionType; } } }
            , MoveDetailsColumn{ MDCOL(TypeReadable, "类型"), SortByAttribute{ [](Action_t& a) { return std::string_view(enum_reflection<EnumParkourAction>::GetString(a->m_ActionType)); } } }
            , MoveDetailsColumn{ MDCOL(Collision, "碰撞层"), SortByAttribute{ [](Action_t& a) { return a->m_CollisionLayer; } } }
            , MoveDetailsColumn{ MDCOL(IsDiscardedImmediately, "立即弃用"), SortByAttribute{ [](Action_t& a) { return a->m_IsDiscarded_immediatelyAfterCreation; } } }
            , MoveDetailsColumn{ MDCOL(Fitness, "适应度"), SortByAttribute{ [](Action_t& a) { return a->m_FitnessWeight; } } }
            , MoveDetailsColumn{ MDCOL(IsDiscardedBecauseFitnessTooLow, "低适应度弃用"), SortByAttribute{ [](Action_t& a) { return a->m_IsDiscarded_becauseFitnessWeightTooLow; } } }
            , MoveDetailsColumn{ MDCOL(DefaultWeight, "默认权重"), SortByAttribute{ [](Action_t& a) { return a->m_DefaultWeight; } } }
            , MoveDetailsColumn{ MDCOL(ModWeight, "调整权重"), SortByAttribute{ [this](Action_t& a) { return GetModWeightForActionType(a->m_ActionType); }}}
            , MoveDetailsColumn{ MDCOL(TotalWeight, "总权重"), SortByAttribute{ [](Action_t& a) { return a->m_TotalWeight; } } }
            , MoveDetailsColumn{ MDCOL(IsChosen, "已选中"), SortByAttribute{ [](Action_t& a) { return a->m_IsTheSelectedBestMatch; } } }
        );
#undef MDCOL
};
}
...
void ParkourDebugWindow::DrawEnforceTab()
{
    ...
    ImGui::HelpMarker(
        "按住 Alt 解锁游戏鼠标。\n"
        "若在指定位置（指定半径内）找到朝向匹配的跑酷动作，\n"
        "将强制选中该动作。\n"
        "可能有效果，也可能没有。"
    );
    {
        bool isNeedToReset = false;
        if (ImGui::Button("清除强制动作"))
            isNeedToReset = true;
        if (isNeedToReset)
        {
    ...
    ImGui::DragFloat("半径", &parkourLog.m_EnforcedMove.m_Radius, 0.0025f, 0.1f, 5.0f);
}
namespace ImGui
{
...
#define ATCOL(colId, name) ActionTypeColumnsIndices::colId, name, DrawCol_##colId
    return std::make_tuple(
        ActionTypesColumn{ ATCOL(TypeInt, "类型ID"), SortByAttribute{ [](ATDesc_t& a) { return a->m_ActionType; }} }
        , ActionTypesColumn{ ATCOL(TypeReadable, "类型"), SortByAttribute{ [](ATDesc_t& a) { return std::string_view(enum_reflection<EnumParkourAction>::GetString(a->m_ActionType)); } } }
        , ActionTypesColumn{ ATCOL(UsesInCycle, "循环内次数"), SortByAttribute{ [GetNumUsesInCycle](ATDesc_t& a) { return GetNumUsesInCycle(a->m_ActionType); }} }
        , ActionTypesColumn{ ATCOL(ModWeight, "调整权重"), SortByAttribute{ [](ATDesc_t& a) { return a->m_ModWeight; } } }
        );
#undef ATCOL
}
...
void ParkourDebugWindow::Draw()
{
    ...
    if (ImGuiCTX::Window _wnd{ "跑酷调试", 0, ImGuiWindowFlags_MenuBar })
    {
        if (ImGuiCTX::MenuBar _menuBar{})
        {
            ImGui::SetNextItemWidth(100.0f);
            ImGui::SliderFloat("不透明度", &opacity, 0.05f, 1.0f);
        }
        if (latestCycle)
            ImGui::Text(
                "时间戳：%llu\n"
        ...
        if (ImGuiCTX::TabBar _tb{ "_pdtb" }) {
            if (ImGuiCTX::Tab _tabSummary{ "概览" }) {
                DrawSummaryTab();
            }
            if (ImGuiCTX::Tab _tabMoveDetails{ "详情" }) {
                DrawDetailsTab();
            }
            if (ImGuiCTX::Tab _tabDisplay{ "动作类型" }) {
                DrawActionTypesTab();
            }
            if (ImGuiCTX::Tab _tabDisplay{ "显示" }) {
                DrawDisplayTab();
            }
            if (ImGuiCTX::Tab _tabDisplay{ "强制" }) {
                DrawEnforceTab();
            }
        }
    }
