...
{
    ImGui::Text("已找到的类型信息：%d", foundTIs.size());
    if (ImGui::Button("查找所有类型信息")) {
        FindAllTypeInfos();
    }
    if (ImGui::Button("复制已找到的类型信息到剪贴板（仅地址）")) {
        ToClipboard_AllTIAddrs();
    }
    if (ImGui::Button("解析所有 TI 并复制到剪贴板")) {
        ToClipboard_ParseAllFoundTypeInfos_NoVTBLs();
    }
    if (ImGui::Button("警告：必定导致崩溃！尝试构造所有对象以查找虚表")) {
        ImGui::OpenPopup("TIS_confirm_construction_crash");
    }
    if (ImGuiCTX::Popup _{ "TIS_confirm_construction_crash" }) {
        ImGui::Text(
            "确定要继续吗？若对全部类型信息执行，之后程序必定崩溃。\n"
            "即使不崩溃，也建议立即重启游戏。\n"
            "崩溃原因：部分构造函数会设置全局变量。\n"
            "我已排除一批绝不尝试构造的类型，以便遍历完整个列表，但崩溃仍会发生。\n"
            "如需完整转储，请从上次失败的类型开始，多重启几次游戏即可，\n"
            "大约只需重复 3-4 次。\n"
            "结果会追加到游戏目录下的 imgui_log.txt 中。"
        );
        static size_t attemptedConstructionFirstIdx = 0;
        static size_t howManyToAttempt = 10000;
        ImGui::InputInt("起始索引##attemptedConstructionFirstIdx", (int*)&attemptedConstructionFirstIdx);
        ImGui::InputInt("尝试数量##howManyToAttempt", (int*)&howManyToAttempt);
        if (ImGui::Button("是，我确定")) {
