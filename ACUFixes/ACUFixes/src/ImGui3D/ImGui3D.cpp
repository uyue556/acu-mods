...
void DrawPersistent3DMarkersControls()
{
    ...
    if (ImGuiCTX::Popup _{ "PersistentMarkerCtxMenu" })
    {
        ...
        ImGui::DragFloat3("位置", m_LocationsWithID[lastSelectedMarkerIDforPopup.value()].m_Params.m_Location, 0.1f);
        if (ImGui::Button("删除"))
        {
...
void ImGui3D::OptionToDrawMultipleVector3fFromClipboard()
{
    ...
    ImGui::Checkbox("仅显示玩家附近", &filterCloseToPlayer);
    ImGui::SliderFloat("过滤半径平方", &filterRadiusSqr, 1, 100);
    if (!ImGui::Button("可视化剪贴板中的多个位置（以换行分隔）"))
    {
        return;
    }
