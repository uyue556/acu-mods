...
void MyAnimationPlayer::DrawTimeSlider()
{
    ...
    if (!animCpnt)
    {
        ...
        ImGui::Text("找不到 AtomAnimComponent");
        return;
    }
    if (!animCpnt->pD0)
    {
        ImGuiCTX::PushStyleColor _redText(ImGuiCol_Text, ImVec4(0.869f, 0.389f, 0.376f, 1.0f));
        ImGui::Text("AtomAnimComponent 的 GraphEvaluation 不存在");
        return;
    }
    ...
    {
        ImGuiCTX::PushStyleColor _redText(ImGuiCol_Text, ImVec4(0.869f, 0.389f, 0.376f, 1.0f));
        ImGui::Text("找不到“CinematicAnimationTime”图变量");
        return;
    }
    float timeMin = 0;
...
void MyAnimationPlayer::DrawControls()
{
    ...
    ImGui::Separator();
    if (picker.Draw("选择要播放的动画", animToRestart))
    {
    ...
    if (ImGui::SliderFloat("速度", &newSpeedMult, -2.0f, 2.0f))
    {
        if (m_playedAnim)
        {
            Pause(*m_playedAnim);
    ...
    if (ImGui::Checkbox("循环", &newIsLooping))
    {
        if (m_playedAnim)
        {
            uint64 animatorTime = GetAnimatorTime();
...
inline bool SliderEulerAngles(const char* label, Vector3f& v)
{
    ...
    value_changed |= SliderScalar("翻滚", data_type, &v.x, &v_min[0], &v_max[0], format, flags);
    PopItemWidth();

    SameLine(0, g.Style.ItemInnerSpacing.x);
    value_changed |= SliderScalar("俯仰", data_type, &v.y, &v_min[1], &v_max[1], format, flags);
    PopItemWidth();

    SameLine(0, g.Style.ItemInnerSpacing.x);
    value_changed |= SliderScalar("偏航", data_type, &v.z, &v_min[2], &v_max[2], format, flags);
    PopItemWidth();
...
public:
    virtual ResultEditedKeyframe_t DrawEditor(byte* trackdata) override
    {
        ...
        auto EditTranslation = [&buf](uint8 keyframeTimeU8, Vector3f& keyframeData) -> ResultEditedKeyframe_t
        {
            ...
            buf.appendf("帧：%d，时间：%f", keyframeTimeU8, keyframeTimeFlt);
...
public:
    virtual ResultEditedKeyframe_t DrawEditor(byte* trackdata) override
    {
        ...
        //}
        {
            ...
            ImGui::Separator();
            ImGui::Text("帧/时间"); ImGui::NextColumn();
            ImGui::Text("数据"); ImGui::NextColumn();
            ImGui::Separator();
...
void DrawSpoofAnimationExperiments()
{
    ...
    ImGuiTextBuffer buf;
    buf.appendf("加载“%s”JSON 动画并开始", quickTestAnimFilepath);
