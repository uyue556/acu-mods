...
void DrawAtomGraphDumper()
{
        ...
        })
    {
        ...
        RaycastPicker_PickAtomGraph();
        {
            ...
            if (ImGui::IsItemHovered())
            {
                ImGui::SetTooltip(
                    "这会让游戏卡顿一小会儿\n"
                    "并在游戏目录生成一个 100MB+ 的 AtomGraphDump.txt 文件。\n"
                    "请做好心理准备。"
                );
            }
        }
    ...
    ImGui::Text("玩家动画图的部分变量：");
    ImGui::SameLine();
    static bool notifyTheGraphWhenModifying = true;
    ImGui::Checkbox("修改时通知动画图", &notifyTheGraphWhenModifying);
    ImGui::Separator();
    // bool IsActing; // 0x1f156dfb/521498107
