...
{
    ImGui::Checkbox("启用慢动作特技", &g_Config.hacks->slowmotionTrick->isActive.get());
    if (ImGui::IsItemHovered())
    {
        ImGui::SetTooltip(
            "按快捷键（默认“鼠标侧键4”）触发一段短暂的慢动作。\n"
            "建议在精彩瞬间出现之后立刻使用。"
        );
    }
    if (g_Config.hacks->slowmotionTrick->isActive)
    {
        ...
        ImGui::DrawEnumPicker("慢动作特技快捷键", g_Config.hacks->slowmotionTrick->hotkey.get(), ImGuiComboFlags_HeightLarge);
    }
}
void DrawSlowMotionControls()
{
    ...
    if (ImGui::SliderFloat("时间流速", &currentTimescale, 0.00f, 2.0f))
    {
