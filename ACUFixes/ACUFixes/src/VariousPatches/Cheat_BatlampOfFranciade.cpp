...
void DrawBatlampControls()
{
    ...
    {
        ImGui::SetTooltip(
            "你可以在“死去的国王”DLC最终任务之外使用圣丹尼斯之首。\n"
            "可选择：\n"
            "- 每次关卡重新加载时自动将普通提灯替换为魔法提灯；\n"
            "- 尝试手动添加魔法提灯。\n"
            "新增快捷键（默认“N”）可进入/退出“可蓄能”模式。\n"
            "若同时启用“假装身处弗朗西亚德”，甚至能在巴黎使用它\n"
            "（副作用是至少会有部分音效缺失）。\n"
            "使用前请务必备份存档。"
        );
    }
    ImGui::Checkbox("用魔法提灯替换普通提灯",
        &g_Config.cheats->batlampOfFranciadeManipulations->doReplaceNormalLampWithMagical.get());
    if (ImGui::IsItemHovered())
    {
        ImGui::SetTooltip(
            "下次关卡重新加载时，普通提灯将被替换为\n"
            "“死去的国王”DLC最终任务中使用的圣丹尼斯之首。"
        );
    }
    ImGui::Checkbox("使用魔法提灯时解除移动限制",
    ...
    if (lanternCpnt)
    {
        ImGui::Checkbox("允许立即蓄能", (bool*)&lanternCpnt->isInModeAbleToCharge_300);
        ImGui::DrawEnumPicker("可蓄能模式切换键",
            g_Config.cheats->batlampOfFranciadeManipulations->batlampChargeModeButton.get(),
            ImGuiComboFlags_HeightLarge);
    }
    ImGui::BulletText("未找到 LanterndlcComponent");
    if (ImGui::Button("尝试手动添加圣丹尼斯之首"))
    {
