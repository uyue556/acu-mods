...
bool BlenderCurveEditor::Draw(ImVec2 editorSize)
{
	...
	ImVec2 visibleRangeY = { m_VisibleRangeLeftBottom.y, m_VisibleRangeRightTop.y };
	{
		const float eps = 0;
		{
			...
			ImGui::SameLine();
			ImGui::PopItemWidth();
			isModified = ImGui::DragFloat("可见范围 X", &visibleRangeX.y, 0.01f, visibleRangeX.x + eps, 2)
				|| isModified;
			ImGui::SameLine();
			ImGui::PopItemWidth();
		...
		}
		{
			...
			ImGui::SameLine();
			ImGui::PopItemWidth();
			isModified = ImGui::DragFloat("可见范围 Y", &visibleRangeY.y, 0.01f, visibleRangeY.x + eps, 2)
				|| isModified;
			ImGui::SameLine();
			ImGui::PopItemWidth();
